...
public class DesignParser {
	...
	public static void generateClassPrintSequence(PrintWriter outputStream, ClassDeclarationVisitor declVisitor,
		...
		for(int i = 1; i < methodVisitor.argumentTypes.size() - 2; i++) {
			outputStream.println(declVisitor.nameGlobal + " -> " + methodVisitor.argumentTypes.get(i) + "[arrowhead=\"onormal\", style=\"dashed\"] ");
			// This should be for the uses section
		}
		
		for(int i = 1; i < fieldVisitor.fieldTypes.size() - 2; i++) {
			outputStream.println(declVisitor.nameGlobal + " -> " + fieldVisitor.fieldTypes.get(i) + "[arrowhead=\"onormal\", style=\"solid\"] ");
			// This should be for the uses section
		}
